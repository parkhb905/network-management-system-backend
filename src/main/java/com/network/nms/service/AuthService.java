...
//                .build();
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        if(user == null) return new LoginResponse(false, "존재하지 않는 사용자입니다.");
        if(!passwordEncoder.matches(request.getPassword(), user.getPassword())) return new LoginResponse(false, "비밀번호가 일치하지 않습니다.");
